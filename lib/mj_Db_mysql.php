...
// simple class wrapper for the mysql database functions
//
 ...
 * Methods that are simple/straight wrappers for underlying mysql_ calls are not documented.
 * The connection is maintained by the class instantiation, method calls to underlying mysql_ functions that take a connection parameter do not accept conn as a param
 *
 * @package mj_lib
 */
class mj_Db 
{
   ...
   * It utilizes mysql_escape_string() while adding some convenience.
   * 
  ...
  static function safeStr($str,$quote=false,$setnull=false)
  {
    ...
    if ($quote) return "'".mysql_escape_string($str)."'";
  ...
  function query($sql,$from="") 
  {
    ...
    if (!($rez = mysql_query($sql,$this->conn)))
    {
      mjlog(ERROR,__METHOD__,"(from:$from) failed query: ".mysql_error($this->conn)."\nsql: $sql",__LINE__);
      if (mysql_errno($this->conn))
      {
        mjlog(WARNING,__METHOD__,"(from:$from) failed query(".mysql_errno($this->conn).'): '.mysql_error($this->conn),__LINE__);
   ...
   */
  function affectedRows() { return mysql_affected_rows($this->conn); }

  function &clientEncoding() { return mysql_client_encoding($this->conn); }
  ...
  function &connect($server, $username, $password, $new_link=false, $client_flags=0)
  {
    ...
    mjlog(DEBUG,__METHOD__,"connect($server,$username,$password)");
    $this->conn = mysql_connect($server, $username, $password, $new_link, $client_flags);
    if (!$this->conn)
    {
      mjlog(ERROR,__METHOD__,"failed to connect",__LINE__);
      mjlog(ERROR,__METHOD__,"failed connect(".mysql_errno($this->conn).'): '.mysql_error($this->conn),__LINE__);
  ...
  function &pconnect($server, $username, $password, $new_link=false, $client_flags=0)
  {
    ...
    return mysql_pconnect($server, $username, $password, $new_link, $client_flags);
  }
  ...
  {
    //Open a persistent connection to a MySQL server
    //return mysql_pconnect($cfg.server, $cfg.username, $cfg.password, $cfg.new_link, $cfg.client_flags);
  }

  function creatDb() { return mysql_create_db($dbname,$this->conn); }
    //Create a MySQL database

  function dataSeek($resouce, $row_number) { return mysql_data_seek($resouce, $row_number); }
    //Move internal result pointer

  function dbName(&$resource) { return mysql_db_name($resource,$index); }
    //Get result data

  function dropDb($dbname) { return mysql_drop_db($dbname,$this->conn); }
  ...
  function beginTransaction() { mysql_query('begin',$this->conn); }
  function endTransaction($action='commit') { mysql_query($action,$this->conn); }
  function commit()   { $this->endTransaction('commit'); }
  function rollback() { $this->endTransaction('rollback'); }

  function escapeString($str) { return mysql_escape_string($str); }
    //Escapes a string for use in a query

  function fetchArray(&$resource, $result_type=MYSQL_ASSOC) { return mysql_fetch_array($resource, $result_type); }
    //Fetch a result row as an associative array, a numeric array, or both

  function fetchAssoc(&$resource) { return mysql_fetch_assoc($resource); }
    //Fetch a result row as an associative array

  function &fetchField(&$resource, $field_offset=0) { return mysql_fetch_field($resource, $field_offset=0); }
    //Get column information from a result and return as an object

  function &fetchLengths(&$resource) { return mysql_fetch_lengths($resource); }
    //Get the length of each output in a result

  function &fetchObject(&$resource, $class_name, $params=NULL) { return mysql_fetch_object($resource, $class_name, $params); }
    //Fetch a result row as an object

  function fetchRow(&$resource) { return mysql_fetch_row($resource); }
    //Get a result row as an enumerated array

  function &fieldFlags(&$resource, $field_offset=0) { return mysql_field_flags($resource, $field_offset); }
    //Get the flags associated with the specified field in a result

  function &fieldLen(&$resource, $field_offset=0) { return mysql_field_len($resource, $field_offset); }
    //Returns the length of the specified field

  function &fieldName(&$resource, $field_offset=0) { return mysql_field_name($resource, $field_offset); }
    //Get the name of the specified field in a result

  function &fieldSeek(&$resource, $field_offset=0) { return mysql_field_seek($resource, $field_offset); }
    //Set result pointer to a specified field offset

  function &fieldTable(&$resource, $field_offset=0) { return mysql_field_table($resource, $field_offset); }
    //Get name of the table the specified field is in

  function &fieldType(&$resource, $field_offset=0) { return mysql_field_type($resource, $field_offset); }
    //Get the type of the specified field in a result

  function freeResult(&$resource) { return mysql_free_result($resource); }
    //Free result memory

  function getClientInfo() { return mysql_get_client_info(); }
    //Get MySQL client info

  function getHostInfo() { return mysql_get_host_info($this->conn); }
    //Get MySQL host info

  function getProtoInfo() { return mysql_get_proto_info($this->conn); }
    //Get MySQL protocol info

  function getServerInfo() { return mysql_get_server_info($this->conn); }
    //Get MySQL server info

  function info() { return mysql_info($this->conn); }
    //Get information about the most recent query

  function insertId() { return mysql_insert_id($this->conn); }
    //Get the ID generated in the last query

  function listDbs() { return mysql_list_dbs($this->conn); }
    //List databases available on a MySQL server

  function listFields($database_name, $table_name) { return mysql_list_fields($database_name, $table_name, $this->conn); }
    //List MySQL table fieldsbase_name , string $table_name

  function listProcesses() { return mysql_list_processes($this->conn); }
    //List MySQL processes

  function listLables($database_name) { return mysql_list_tables($database_name, $this->conn); }
    //List tables in a MySQL database

  function numFields(&$resource) { return mysql_num_fields($resource); }
    //Get number of fields in result

  function numRows(&$resource) { return mysql_num_rows($resource); }
    //Get number of rows in result

  function ping() { return mysql_ping($this->conn); }
    //Ping a server connection or reconnect if there is no connection

  function realEscapeString() { return mysql_real_escape_string($str,$this->conn); }
    //Escapes special characters in a string for use in an SQL statement

  function &result(&$resource, $row, $field=0 ) { return mysql_result($resource, $row, $field); }
    //Get result data

  function selectDb($dbname) 
  { 
    //Select a MySQL database within the current connection
    //
    mjlog(DEBUG,__METHOD__,"select database [$dbname]");

    if ($this->currentDb == $dbname) return true;
    
    if (!($rez = mysql_select_db($dbname,$this->conn)))
  ...
  function setCharset($charsetstr) { return mysql_set_charset($charsetstr,$this->conn); }
    //Sets the client character set

  function stat() { return mysql_stat($this->conn); }
    //Get current system status

  function threadId() { mysql_thread_id($this->conn); }
    //Return the current thread ID

  function &unbufferedQuery($sql) { return mysql_unbuffered_query($sql,$this->conn); }
  ...
  /**
   * convenience function that creates the field list and value list for including in an insert function.
   */
  public function makeInsertParts($fields,$data)
  {
    $names  = '';
    $values = '';
    $comma  = '';
    ...
    foreach ($fields as $name)
    {
      ...
      {
        $names  .= $comma . '`'. $name .'`';
        $values .= $comma . $this->safeStr(trim($data[$name]),true);
        $comma   = ',';
      }
    }
   ...
   * convenience function that creates the field list and value list for including in an insert function.
   */
  public function makeUpdateParts($fields,$data,$validation)
  {
    $sets  = '';
    $comma  = '';
    ...
    foreach ($fields as $name)
    {
      if (isset($data[$name]) && trim($data[$name]) != '')
      {
        if (!isset($validation[$name]) || $validation[$name] != ($data[$name]))
        {
          $sets .= $comma . $name ." = ". $this->safeStr($data[$name],$quoteString=true); 
          $comma = ',';
        }
      }
    }
 ...
 /**
  ...
  {
    $fields = array();
    $nfields = mysql_num_fields($rez);
  
    for ($ii = 0; $ii < $nfields; $ii++)
    {
      $fields[] = mysql_field_name($rez, $ii);
    }
    return $fields;
  }
